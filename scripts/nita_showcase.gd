...
@onready var nita_front: Node2D = get_node_or_null("NitaFrontView")
@onready var nita_side: CharacterBody2D = get_node_or_null("NitaSideRig")
@onready var expr_label: Label = get_node_or_null("ExpressionStatus")

var expressions := ["grin", "angry", "happy", "shocked", "hurt", "neutral"]
var expr_idx := 0
var timer := 0.0
...
func _ready() -> void:
	...
	if nita_side:
		nita_side.set_physics_process(false)
		var anim = nita_side.find_child("AnimPlayer", true, false)
		if anim:
			anim.play("idle")
	_update_expression()
...
func _update_expression() -> void:
	...
	if nita_front:
		var face = nita_front.find_child("Face", true, false)
