...
func _physics_process(delta: float) -> void:
	...
	# Maximum range / lifetime expiration
...
func _handle_collision(collider: Node) -> void:
	...
	if collider.has_method("take_hit") or collider.is_in_group("hit_receiver"):
		is_active = false
		var hit = HitDataClass.create(damage, direction, knockback_force, source, "LEON_BASIC", global_position)
